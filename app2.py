...
# Load the data
df = pd.read_csv('user_1_108_values.csv')
...
def draw_room_with_obstacles_and_path(width, height, obstacles, step_numbers, start_point, destination_step, path):
    ...
    ax.set_xlabel('')
    ax.set_ylabel('')
    ax.set_title('Project and Design Lab')
    ...
    for step, (x, y) in step_numbers.items():
        ...
        ax.text(block_center_x, block_center_y, str(step), ha='center', va='center', fontsize=8, color='none')
        if step == start_point:
            ax.text(block_center_x, block_center_y - 0.5, 'Start', ha='center', va='center', fontsize=12, color='blue')
        if step == 52:
            ax.text(block_center_x, block_center_y - 0.5, 'Door', ha='center', va='center', fontsize=12, color='green')
        if step == 80:
            ax.text(block_center_x, block_center_y - 0.5, 'AP2', ha='center', va='center', fontsize=12, color='green')
        if step == 53:
            ax.text(block_center_x, block_center_y - 0.5, 'AP1', ha='center', va='center', fontsize=12, color='green')
        if step == 31:
            ax.text(block_center_x, block_center_y - 0.5, 'Server', ha='center', va='center', fontsize=12, color='green')
        if step == 27:
            ax.text(block_center_x, block_center_y - 0.5, 'Sol. st.', ha='center', va='center', fontsize=12, color='green')
        if step == 9:
            ax.text(block_center_x, block_center_y - 0.5, 'Pcb', ha='center', va='center', fontsize=12, color='green')
        if step == 1:
            ax.text(block_center_x, block_center_y - 0.5, 'AP4', ha='center', va='center', fontsize=12, color='green')
        if step == 81:
            ax.text(block_center_x, block_center_y - 0.5, 'WB', ha='center', va='center', fontsize=12, color='green')
...
def predict():
    destinations = {"door": 52,
                    "access point 2": 80,
                    "access point 1": 53,
                    "server": 31,
                    "soldering station": 27,
                    "pcb manufacturing": 9,
                    "access point 4": 1,
                    "white board": 81  
                    }
    ...
    destination_step = destinations[destination]
    for network in networks:
        rss_values[network[0]] = network[1]
    Wifi_1 = rss_values["Wifi_1"]
    Wifi_2 = rss_values["Wifi_2"]
    Wifi_3 = rss_values["Wifi_3"]
    Wifi_4 = rss_values["Wifi_4"] 
    Wifi = [Wifi_1, Wifi_2, Wifi_3, Wifi_4]
    ...
    predicted_step = predict_step(Wifi)
    ...
    return render_template('result.html', predicted_step=predicted_step, path=path, curr_loc=Wifi)
