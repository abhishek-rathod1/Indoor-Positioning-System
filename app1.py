...
def predict():
    ...
    rss_values = [
        int(request.form['wifi_1']),
        int(request.form['wifi_2']),
        int(request.form['wifi_3']),
        int(request.form['wifi_4'])
    ]
    destination_step = int(request.form['destination'])

    # Predict the starting step based on RSS values
    predicted_step = predict_step(rss_values)
    ...
    return render_template('result.html', predicted_step=predicted_step, path=path)
