...
def draw_room_with_obstacles_and_path(width, height, obstacles, step_numbers, start_point, destination_step, path):
    ...
    ax.set_xticks(range(0, width*2 + 1, ))

    # Set y-axis ticks from 0 to height with step size 1
    ax.set_yticks(range(height*2 + 1))
    ...
    for step, (x, y) in step_numbers.items():
        ...
        if step == 52:
            ax.text(block_center_x, block_center_y - 0.5, 'Door', ha='center', va='center', fontsize=8, color='green')
        if step == 80:
            ax.text(block_center_x, block_center_y - 0.5, 'AP2', ha='center', va='center', fontsize=8, color='green')
        if step == 53:
            ax.text(block_center_x, block_center_y - 0.5, 'AP1', ha='center', va='center', fontsize=8, color='green')
        if step == 31:
            ax.text(block_center_x, block_center_y - 0.5, 'Server', ha='center', va='center', fontsize=8, color='green')
        if step == 27:
            ax.text(block_center_x, block_center_y - 0.5, 'Sol. st.', ha='center', va='center', fontsize=8, color='green')
        if step == 9:
            ax.text(block_center_x, block_center_y - 0.5, 'Pcb', ha='center', va='center', fontsize=8, color='green')
        if step == 1:
            ax.text(block_center_x, block_center_y - 0.5, 'AP4', ha='center', va='center', fontsize=8, color='green')
        if step == 81:
            ax.text(block_center_x, block_center_y - 0.5, 'WB', ha='center', va='center', fontsize=8, color='green')
        # if destination_label[step]:
        #   ax.text(block_center_x, block_center_y, destination_label[step], ha='center', va='center', fontsize=8, color='black')
